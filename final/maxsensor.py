...
m = MAX30102() # sensor initialization
red, ir = m.read_sequential()
dict_data = {}
name = input("enter the name")
val = input("testing before food yes/No")
dict_data["Name"] = name
dict_data["Fasting"] = val
dict_data["Time"] = str(datetime.now())
dict_data["Red"]=str(list(red))
dict_data["Ir"] = str(list(ir))
print(dict_data)

res = requests.post('https://oxygenlevel-reading.herokuapp.com/Update', json=dict_data)
if res.ok:
    print(res.content)

#data=pd.DataFrame.from_dict(dict_data)

#new_row = [name,datetime.now(),val,list(red),list(ir)]

#dict = {'name': name, 'time': datetime.now(), 'before food': val,"red":list(red),"ir": list(ir)} 
